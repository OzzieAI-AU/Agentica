    ...
    public class WorkerAgent : BaseAgent
    {
        ...
        protected override async Task ProcessIncomingMessageAsync(AgentMessage message)
        {
            ...
            if (message.Type != MessageType.TaskAssignment)
            {
                ...
                ConsoleLogger.WriteLine($"[Worker {Config.Name}] ⚠️ Received unexpected message type: {message.Type}", ConsoleColor.Yellow);
                return;
            }

            // 2. Beautiful success log indicating a new task has arrived
            ConsoleLogger.WriteLine($"[Worker {Config.Name}] ?? RECEIVED NEW TASK", ConsoleColor.Green);
            ...
            try
            {
                ...
                ConsoleLogger.WriteLine($"[Worker {Config.Name}] ✅ TASK COMPLETED SUCCESSFULLY", ConsoleColor.Green);

                // 8. Display the final report for immediate visibility
                ConsoleLogger.WriteLine($" Final Report:\n{finalReport}", ConsoleColor.Green);

                // 9. Determine the recipient — prefer configured ParentId, fallback to original sender
                string targetRecipient = Config.ParentId ?? message.SenderId;

                // 10. Send the final result back to the Manager (or designated parent)
                ...
                ConsoleLogger.WriteLine($"[Worker {Config.Name}] 📤 Sent final result to Manager", ConsoleColor.DarkYellow);
            }
            catch (Exception ex)
            {
                ...
                ConsoleLogger.WriteLine($"[Worker {Config.Name}] 💥 ERROR processing task: {ex.Message}", ConsoleColor.Red);
            }
        }
    }
}